...
public class MiniMap
{
    ...
    {
        for (int v = topLeft.x * 32; v <= AppWindow.WINDOW_WIDTH - (bottomRight.x * MINI_MAP_TILE_WIDTH); v += MINI_MAP_TILE_WIDTH)
            GraphicsDraw.addLine2D(new Vector2i(v, topLeft.y * MINI_MAP_TILE_HEIGHT), new Vector2i(v, AppWindow.WINDOW_HEIGHT - (bottomRight.y * MINI_MAP_TILE_HEIGHT)), Color.WHITE);
        for (int h = topLeft.y * 32; h <= AppWindow.WINDOW_HEIGHT - (bottomRight.y * MINI_MAP_TILE_HEIGHT); h += MINI_MAP_TILE_HEIGHT)
            GraphicsDraw.addLine2D(new Vector2i(topLeft.x * MINI_MAP_TILE_WIDTH, h), new Vector2i(AppWindow.WINDOW_WIDTH - (bottomRight.x * MINI_MAP_TILE_WIDTH), h), Color.WHITE);
    }
