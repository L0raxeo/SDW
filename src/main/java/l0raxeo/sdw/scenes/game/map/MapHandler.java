...
public class MapHandler
{
    ...
    //              tileUnits        [           24           ][           24            ]
    //              pixels           [          1536          ][          1536           ]
    private final BufferedImage[][] tileMap = new BufferedImage[AppWindow.WINDOW_WIDTH / 32][AppWindow.WINDOW_HEIGHT / 32];
    public final int TILE_WIDTH = 64, TILE_HEIGHT = 64;
    public final int TILE_MAP_SIZE_X = 32, TILE_MAP_SIZE_Y = 32;

    public void generateTileMap()
    {
        for (int r = 0; r < AppWindow.WINDOW_HEIGHT / TILE_MAP_SIZE_Y; r++)
            for (int c = 0; c < AppWindow.WINDOW_WIDTH / TILE_MAP_SIZE_X; c++)
        ...
        for (int r = 0; r < AppWindow.WINDOW_HEIGHT / TILE_MAP_SIZE_Y; r++)
            for (int c = 0; c < AppWindow.WINDOW_WIDTH / TILE_MAP_SIZE_X; c++)
            {
                if ((TILE_WIDTH * c) + Camera.xOffset() > -TILE_WIDTH &&
                        (TILE_WIDTH * c) + Camera.xOffset() < AppWindow.WINDOW_WIDTH + TILE_WIDTH &&
                        (TILE_HEIGHT * r) + Camera.yOffset() > -TILE_HEIGHT &&
                        (TILE_HEIGHT * r) + Camera.yOffset() < AppWindow.WINDOW_HEIGHT + TILE_HEIGHT)
