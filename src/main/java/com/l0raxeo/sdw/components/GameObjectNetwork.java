...
public class GameObjectNetwork extends Component implements PacketHandler
{
    ...
    /*
    Packet Format:
    packetId,gameObjectUid,xPos,yPos,xSize,ySize,isDead
    Example:
    gon,0,64,64,16,16,false
     */
    ...
    public void receive(String packet) {
        ...
        handleParsedPacket(packetId, gameObjectUid, position, scale, isDead);
    }
    ...
    private void handleParsedPacket(String packetId, int gameObjectUid, Vector2f position, Vector2f scale, boolean isDead)
    {
        if (!packetId.equals("gon"))
            throw new PacketMismatchException("Received packet of ID '" + packetId + "' in a GameObjectNetwork that only receives packets of ID 'gon'");
        else if (gameObjectUid != gameObject.getUid())
